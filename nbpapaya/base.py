...
class Overlay(ViewerBase):
    def __init__(self, image_options, num=None, options=None,
        ...
        #         width, height, host)
        super(Overlay, self).__init__(fnames, num, options, image_options,
                 width, height)
        new_mapper = {}
        print(self._mapper)

        for key, value in self._mapper.items():
            newkey = "/files/{}".format(value)
            print(newkey)
            new_mapper[newkey] = image_options[key]
            new_mapper[newkey]["filename"] = "/files/{}".format(list(self._symlink_files([image_options[key]["filename"]])[1].values())[0])
     ...
     <html lang="en">
       <head>
         ...
         <body ng-app="visApp">
             ...
             <script src="https://cdnjs.cloudflare.com/ajax/libs/dat-gui/0.6.3/dat.gui.js"></script>
...
<script>
             //if ( ! Detector.webgl ) Detector.addGetWebGLMessage();
                ...
                var calculateFaceMetrics = function(mesh, callback){
                    ...
                    console.log("csv filename is", csv_filename)
				...
				var color_brain = function(mesh){
                    console.log('coloring_brain')
                    var mesh_options = mesh.anisha_opts
                    var key = mesh_options.key
                    var face_metrics = mesh.face_metrics[key]
                    console.log("face metrics are",face_metrics)
